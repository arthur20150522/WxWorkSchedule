...
class BridgeHandler(BaseHTTPRequestHandler):
    ...
    def _handle_wechat_launch(self):
        ...
        try:
            ...
            # Fallback: try explorer shell:AppsFolder
            try:
                subprocess.Popen(
                    'explorer shell:AppsFolder\\Microsoft.AutoGenerated.{5FD21F2E-64C7-8F20-F9FB-2514D0A0A64B}',
                    shell=True
                )
                log.info('[wechat-ctrl] Launched WeChat via shell:AppsFolder')
                self._send({'success': True, 'launched': True, 'path': 'shell:AppsFolder'})
                return
            except Exception:
                pass
