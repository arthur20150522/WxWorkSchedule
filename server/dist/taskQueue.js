...
export function calculateNextTime(task) {
    ...
    switch (task.recurrence) {
        ...
        case 'interval': {
            ...
            switch (task.intervalUnit) {
                case 'minute':
                    next = addMinutes(current, val);
                    break;
                case 'hour':
                    next = addHours(current, val);
                    break;
                case 'day':
                    next = addDays(current, val);
                    break;
            }
            break;
        }
    ...
    while (next <= now && iterations < 1000) {
        ...
        switch (task.recurrence) {
                ...
                if (slots.length > 0) {
                    ...
                    for (const slot of slots) {
                        ...
                        for (const d of (slot.days || [])) {
                            ...
                            candidate.setDate(candidate.getDate() + diff);
                            candidate.setHours(h || 9, m || 0, 0, 0);
                            if (candidate > next && (!best || candidate < best))
                                best = candidate;
                        }
                    }
                    if (best) {
            ...
            case 'interval': {
                ...
                switch (task.intervalUnit) {
                    case 'minute':
                        next = addMinutes(next, val);
                        break;
                    case 'hour':
                        next = addHours(next, val);
                        break;
                    case 'day':
                        next = addDays(next, val);
                        break;
                    default:
                        next = addDays(next, 1);
...
class TaskQueue {
    ...
    _currentTarget = null;
    _lastError = null;
    _addingIds = new Set(); // track in-flight add() to prevent scheduler race
    ...
    async add(task) {
        ...
        if (this.queue.find(t => t.id === task.id))
            return;
        if (this._addingIds.has(task.id))
            return;
    ...
    async executeTask(task) {
        ...
        try {
            ...
            if (fresh)
                currentIndex = fresh.currentContentIndex || 0;
        }
