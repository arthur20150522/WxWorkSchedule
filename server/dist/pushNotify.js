...
async function pushChuckFang(title, message, linkUrl) {
    ...
    if (linkUrl) {
        try {
            ...
            console.error('[Push:鸿蒙] html push error:', e.message);
        }
...
async function pushServerJiang(title, message, linkUrl) {
    try {
...
export async function pushNotify(title, message, linkUrl) {
    if (!PUSH_ENABLED)
        return;
