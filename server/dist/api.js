...
// Trigger bridge auto-recovery (dismiss popup + click login)
apiRouter.post('/bridge/recover', async (req, res) => {
    try {
...
apiRouter.get('/search', async (req, res) => {
    ...
    try {
        const results = await wxBridge.search(q, type);
        res.json({ results });
    }
    catch (e) {
        handleError(res, e);
...
apiRouter.post('/templates', async (req, res) => {
    try {
        const user = req.user;
        const db = await DBManager.getDb(user);
        const template = { id: Date.now().toString(), ...req.body, createdAt: new Date().toISOString() };
        await db.update(({ templates }) => { if (!templates)
...
apiRouter.delete('/templates/:id', async (req, res) => {
    try {
        ...
        const db = await DBManager.getDb(user);
        const { id } = req.params;
        await db.update(({ templates }) => { const idx = templates.findIndex(t => t.id === id); if (idx > -1)
    ...
    try {
        ...
        await db.update(({ tasks }) => {
            ...
            if (index > -1) {
                ...
                const merged = { ...existing, ...req.body, id: existing.id, createdAt: existing.createdAt, updatedAt: new Date().toISOString() };
...
apiRouter.delete('/tasks/:id', async (req, res) => {
    try {
        ...
        if (deleted) {
            await addLog(user, 'info', `Task ${id} deleted from ${ip}`);
            res.json({ success: true });
        }
        else
            res.status(404).json({ error: 'Task not found' });
