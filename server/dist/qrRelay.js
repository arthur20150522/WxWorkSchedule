...
let lastClearedToken = null;
let lastClearReason = null;
function getOrCreateLiveToken() {
    if (!liveToken)
        liveToken = randomBytes(16).toString('hex');
    return liveToken;
}
function liveFilePath() {
    return liveToken ? join(QR_DIR, `live-${liveToken}.png`) : '';
}
function clearLive(reason) {
...
function deadLinkPageHtml(title, detail) {
...
export function getQrViewUrl() {
    return `${PUBLIC_BASE}/api/qr/view/${getOrCreateLiveToken()}`;
}
function viewPageHtml(token) {
    ...
    app.get('/api/qr/view/:token', (req, res) => {
        ...
        if (!/^[a-f0-9]{32}$/.test(token) || token !== liveToken) {
            ...
            if (token === lastClearedToken && lastClearReason === 'recovered') {
                res.status(404).type('html').send(deadLinkPageHtml('微信已恢复，无需扫码 ✅', '这个二维码链接在你扫码成功后已被服务器主动作废（安全设计）。<br>当前微信登录正常。'));
            }
            else {
                res.status(404).type('html').send(deadLinkPageHtml('链接已失效', '二维码会话已结束（超过 5 分钟无刷新，或服务重启）。<br>如微信仍未登录，请等待下一次掉线推送的新链接。'));
            }
            return;
        }
